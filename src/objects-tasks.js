...
function mergeObjects(arr) {
  return arr.reduce((a, b) => {
    return Object.entries(b).reduce(
      (acc, entry) => ({
        ...acc,
        [entry[0]]: acc[entry[0]] ? entry[1] + acc[entry[0]] : entry[1],
      }),
      a
    );
  }, {});
...
function makeImmutable(obj) {
  return Object.freeze(shallowCopy(obj));
}
...
function makeWord(o) {
  return isEmptyObject(o)
    ? ''
    : new Array(Math.max(...Object.values(o).flat()) + 1)
        .fill(null)
        .reduce(
          (a, _, i) => a + Object.keys(o).find((ch) => o[ch].includes(i)),
          ''
        );
}
...
function sellTickets(queue) {
  const cash = [];
  return queue.every((el) => {
    const isChange = cash.reduce((a, b) => a + b, 0) >= el - 25;
    cash.push(el);
    return isChange;
  });
}
 ...
 *   }
 */
function group(arr, keyF, valF) {
  const map = new Map([...new Set(arr.map(keyF))].map((el) => [el, []]));
  arr.map((el) => map.set(keyF(el), [...map.get(keyF(el)), valF(el)]));
  return map;
}
